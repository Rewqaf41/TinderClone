...
export function PhotoGrid({
  ...
  return (
    <div className="grid grid-cols-3 gap-4 px-2 w-full max-w-[396px]">
      {Array.from({ length: maxPhotos }).map((_, index) => {
        const photo = photos[index]
        return (
          <div
            key={index}
            className="relative aspect-3/4 bg-[#1C1F26] rounded-2xl"
          >
            {photo ? (
            ...
            {photo ? (
              ...
              <label
                className={`absolute inset-0 flex items-center justify-center border-dashed border-3 border-border rounded-2xl cursor-pointer ${isDragging ? 'border-primary-dark' : ''}`}
                onDragOver={e => {
                  e.preventDefault()
                  setIsDragging(true)
                }}
              ...
              >
                <input
                  ...
                  className="hidden"
                />
                <Badge className="absolute bottom-0 -right-1 z-10">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 16 16"
